...
async function logMeIn(username, password) {
	...
	switch (response.data.message) {
		...
		case "Logged in":
			...
			break;
		default:
			toast.fire({ icon: "error", text: "an unexpected error occured" });
	}
}
...
async function registerMe(registerInfo, props) {
	...
	const user = await axios.post("auth/users/register", {
		username,
		password,
		email,
	});
...
async function checkUserExists() {
	await axios.get("/auth/user").then(async (res) => {
		...
		const accountBalance = await accountFunctions.getCurrentFunds(user_id);
		this.setState({
			username,
			user_id,
			accountBalance,
